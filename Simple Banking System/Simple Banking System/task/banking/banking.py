...
def CreateAccount():
    ...
    for i in range(0, 9):
        ...
        if i % 2 == 0:
            if random_num > 4:
                accumulative_num += random_num * 2 - 9
            else:
                accumulative_num += random_num * 2
        else:
            accumulative_num += random_num
    account += str((10 - accumulative_num % 10) % 10)
    for i in range(0, 4):
...
def Transference(log_account):
    ...
    elif luhn_check(int(transfer_account)) is False:
